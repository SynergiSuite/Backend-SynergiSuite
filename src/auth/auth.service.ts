...
export class AuthService {
  ...
  async forgotPassword(data: ForgotPasswordDto) {
    const redisData =  await this.getCodeFromRedis(data.email);
    if (!redisData) throw new HttpException('Could not find any data on this email. Please try again later', 400);

    if (redisData.otp !== data.userCode.toString()) throw new HttpException('Invalid Code', 401);

    const user = await this.userService.findByEmail(data.email);
    const validate = await bcrypt.compare(data.updatedPassword, user.password_hash);
    if (validate) throw new HttpException("New password cannot be same as old password.", HttpStatus.BAD_REQUEST);
  ...
  async logout(data: any) {
    try {
      await this.userRepository.update({email: data.email}, {token_digest: null});
      return {
